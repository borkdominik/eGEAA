		...
		*/
		...
		/*for (EObject r : flatTree) {
			if(!(r instanceof IFolder
					|| r instanceof IDiagramModel
					|| r instanceof DiagramModelArchimateObject
					|| r instanceof DiagramModelBendpoint
					|| r instanceof DiagramModelArchimateConnection
					|| r instanceof DiagramModelNote
					|| r instanceof DiagramModelReference
					|| r instanceof Bounds
		            || r instanceof IArchimateRelationship 
		            || r instanceof ArchimateModel))
				this.createNode(r, graph);
		}*/
		...
		/*for(IFolder folder : model.getFolders()) {
			this.createSubgraph(folder, graph);
		}*/
...
protected void createSubgraph(IDiagramModel model, Graph graph) {
	...
	protected void createDotReference(EObject source, EObject target, EReference ref, Graph graph) {
		...
		if(source instanceof IArchimateRelationship) {
			...
			edge.setSource((ConnectableElement)getObjectById(getObjectID(((IArchimateRelationship) source).getTarget())));
		}
		else {
			edge.setName(ref.eClass().getName());
		...
		if(target instanceof IArchimateRelationship) {
			...
			edge.setTarget((ConnectableElement)getObjectById(getObjectID(((IArchimateRelationship) target).getTarget())));
		}
		else {
			edge.setName(ref.eClass().getName());
